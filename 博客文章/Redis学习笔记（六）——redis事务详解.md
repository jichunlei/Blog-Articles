...
* 而当客户端处于事务状态时，服务器收到来自客户端的命令时不会立即执行，而是将命令放入事务队列中去，然后返回`QUEUED`
...
* `DISCARD` :表示取消事务，执行后将清空整个事务队列，然后将客户端从事务状态调整回非事务状态，最后返回`OK`，表示事务已被取消。
...
* Redis 的事务保证了 ACID 中的**一致性（C）**和**隔离性（I）**，但并不保证原子性（A）和持久性（D）。
